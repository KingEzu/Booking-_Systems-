...
export const addShow = async (req, res) =>{
    try{
        const {movieId, showInput, showPrice, type} = req.body;
        ...
        if(!movie){
          ...
          const showsToCreate = [];
                const halls = ["C1", "C2", "C3"];
            
         let showArray = showInput;
        if (!Array.isArray(showArray)) {
            showArray = [showArray];
        }
        
        showArray.forEach(show => {
            const showDate = show.date;
            show.time.forEach((time, index) => {
